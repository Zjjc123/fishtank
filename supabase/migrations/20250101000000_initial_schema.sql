...
CREATE INDEX IF NOT EXISTS idx_user_fish_user_id ON user_fish(user_id);
CREATE INDEX IF NOT EXISTS idx_user_fish_rarity ON user_fish(fish_rarity);
CREATE INDEX IF NOT EXISTS idx_user_fish_date_caught ON user_fish(date_caught);
...
CREATE POLICY "Users can insert their own fish" ON user_fish
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own fish" ON user_fish
    FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Users can delete their own fish" ON user_fish
...
GRANT ALL ON user_fish TO authenticated; 